...
serve(async (req) => {
  ...
  const [prefix, stockCode, actionCode, sharesText] = String(callback.data).split("|");
  const action = ACTIONS[actionCode];
  const actionShares = Number.parseInt(sharesText, 10);

  if (prefix !== "exec" || !stockCode || !action || !Number.isFinite(actionShares) || actionShares <= 0) {
  ...
  const sharesDelta = action.sign < 0
    ? -Math.min(actionShares, sharesBefore)
    : actionShares;
