...
class GeneratorReportTest(unittest.TestCase):
    ...
    def test_v21_1_strong_rebound_uses_multi_window_retest_context(self):
        ...
        self.assertIn("卡關主因：急彈未回測", card)
        self.assertIn("量化差距：急彈追價區，尚未回測｜回測區 118~118.59｜V10 0.44x / V20 0.62x偏弱｜品質 D 未達B｜理論RR 1.6僅參考", card)
        self.assertIn("解鎖：回測區 118~118.59不破 + 非漲停追價 + 量能有效 + 品質B以上 + RR>=1.5", card)
    ...
    def test_v21_1_retest_anchor_says_breakout_zone_when_price_is_below_zone(self):
        ...
        self.assertIn("突破區 118~118.59（現價未站回）", card)
        self.assertIn("解鎖：先站回突破區 118~118.59，再回測不破 + 非漲停追價 + 量能有效 + 品質B以上 + RR>=1.5", card)
        self.assertNotIn("解鎖：突破區 118~118.59（現價未站回）不破", card)
    ...
    def test_afterhours_cards_are_denoised_without_first_read_preface(self):
        ...
        self.assertIn("卡關主因：", unheld_card)
        self.assertIn("量化差距：", unheld_card)
    ...
    def test_v20_4_21_afterhours_mobile_readability_probe(self):
        ...
        self.assertNotRegex(position, r"數據：RR 2\.73")
        self.assertIn("補充：理論RR僅參考，但熱度未降溫", unheld)
    ...
    def test_report_score_evidence_display_owner_specimen_message_list_replay(self):
        ...
        self.assertIn("補充：理論RR僅參考；量能達標，但熱度未降溫", overheat_card)
    ...
    def test_v20_4_42_postmarket_unheld_gate_attribution_readability_message_list_replay(self):
        ...
        self.assertIn(
            "量化差距：RR 0.98｜需>=1.5｜差0.52｜距突破 6%｜突破買點區需<=5%｜差1%｜若走趨勢延續/回測承接，需另見有效setup",
            rr_card,
        )
        ...
        self.assertIn("量化差距：距突破區 9%｜突破買點區需<=5%｜差4%", failed_card)
