...
class GeneratorReportTest(unittest.TestCase):
    def test_v20_4_38_rr_wait_card_reason_and_backtest_summary_readability(self):
        ...
        self.assertIn("【06/04 盤中｜v20.4.46】", summary)
    ...
    def test_generate_report_fails_closed_when_holdings_source_missing(self):
        ...
        self.assertIn("｜v20.4.46】", summary)
    ...
    def test_v20_4_46_future_30d_watch_optional_fourth_message_official_list(self):
        holding_payload = render_payload(
            [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119],
            {"shares": 50, "avg_price": 118},
            price=119,
        ...
        global_events = [
            ...
            {"date": "2026/06/15-16", "event": "BOJ MPM", "impact": "利率/匯率", "source": "BOJ"},
        ...
        global_lines = [
            ...
            if "｜影響面：" in line and "source=" in line
        ]
        self.assertEqual(len(global_lines), 5)
        self.assertEqual(global_lines[0], "06/10-11 ECB 利率會議/記者會｜影響面：利率/匯率｜source=ECB")
        self.assertIn("06/10 美國 CPI（May 2026）｜影響面：通膨/利率｜source=BLS", global_lines[1])
        self.assertIn("06/15-16 BOJ MPM｜影響面：利率/匯率｜source=BOJ", global_lines)
        self.assertIn("06/16-17 Fed FOMC SEP｜影響面：利率/匯率｜source=Fed", global_lines)
    ...
    def test_v20_4_46_future_watch_global_event_ranges_sort_and_fail_closed(self):
        payload = {
        ...
        self.assertEqual(global_lines[0], "06/10-11 ECB 利率會議/記者會｜影響面：利率/匯率｜source=ECB")
        self.assertEqual(global_lines[1], "06/10 美國 CPI（May 2026）｜影響面：通膨/利率｜source=BLS")
        self.assertEqual(global_lines[2], "06/15-16 BOJ MPM｜影響面：利率/匯率｜source=BOJ")
        self.assertIn("06/15-17 G7 Evian｜影響面：政治風險｜source=G7", global_lines)
        self.assertIn("06/16-17 Fed FOMC SEP｜影響面：利率/匯率｜source=Fed", global_lines)
        self.assertNotIn("Invalid official row", message)
        self.assertNotIn("Bad date", message)

    def test_v20_4_46_live_twse_source_is_readonly_and_fails_closed_to_no_match(self):
        responses = {
            "MI_INDEX": [
                {"指數": "發行量加權股價指數", "收盤指數": "45,677.46", "漲跌百分比": "-1.68"},
            ],
            "MI_5MINS_HIST": [
                {"Date": "1150601", "ClosingIndex": "45337.91"},
                {"Date": "1150602", "ClosingIndex": "45557.31"},
                {"Date": "1150603", "ClosingIndex": "46459.16"},
                {"Date": "1150604", "ClosingIndex": "45677.46"},
            ],
        }
        ...
        self.assertEqual(source["status"], "insufficient-data")
    ...
    def test_v20_4_46_live_mops_adapter_blocks_unparseable_spa_shell(self):
    ...
    def test_v20_4_46_live_mops_adapter_fails_closed_on_malformed_table(self):
        ...
        self.assertIn("source-error（MOPS）", message)

    def test_v20_4_46_live_mops_adapter_parses_future_official_table_rows(self):
        html = """
    ...
    def test_v20_4_46_live_global_events_parse_official_pages_or_seed_fallback(self):
        pages = {
    ...
    def test_v20_4_46_future_30d_watch_fails_closed_without_fake_events(self):
        holding_payload = render_payload(
            [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119],
            {"shares": 50, "avg_price": 118},
            price=119,
        ...
        self.assertIn("法說會提醒：source-error（MOPS），本次不列事件", watch)
        self.assertIn("全球事件：source-error，本次不列未確認事件", watch)
        self.assertNotIn("金融海嘯急跌", watch)
        self.assertNotIn("即將崩盤", watch)
        self.assertNotIn("重演", watch)
        self.assertNotIn("新倉建議", watch)

    def test_v20_4_46_generate_report_appends_live_readonly_future_watch_sources(self):
        holding_payload = render_payload(
            [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119],
            {"shares": 50, "avg_price": 118},
            price=119,
        ...
        def future_sources(now):
            return {
                "historical_source": {
                    ...
                    "line": "歷史類比：無高相似崩盤樣本｜依據不足/相似度低｜source=TWSE",
                },
                "mops_adapter": lambda _params: {"status": "source-error"},
                "global_event_source": {
                    "status": "available",
                    "events": [
                        {"date": "2026/06/10-11", "event": "ECB monetary policy meeting/press conference", "impact": "利率/匯率", "source": "ECB"},
                        {"date": "2026/06/10", "event": "美國 CPI（May 2026）", "impact": "通膨/利率", "source": "BLS"},
                        {"date": "2026/06/15-16", "event": "BOJ MPM", "impact": "利率/匯率", "source": "BOJ"},
                        {"date": "2026/06/15-17", "event": "G7 Evian", "impact": "政治風險", "source": "G7"},
                        {"date": "2026/06/16-17", "event": "Fed FOMC SEP", "impact": "利率/匯率", "source": "Fed"},
                        {"date": "2026/06/18", "event": "BoE MPC", "impact": "利率/匯率", "source": "BoE"},
                    ],
                },
        ...
        self.assertIn("歷史類比：無高相似崩盤樣本｜依據不足/相似度低｜source=TWSE", watch)
        self.assertIn("法說會提醒：source-error（MOPS），本次不列事件", watch)
        global_lines = [
        ...
        self.assertEqual(len(global_lines), 5)
        self.assertEqual(global_lines[0], "06/10-11 ECB monetary policy meeting/press conference｜影響面：利率/匯率｜source=ECB")
        self.assertEqual(global_lines[1], "06/10 美國 CPI（May 2026）｜影響面：通膨/利率｜source=BLS")
        self.assertIn("06/15-16 BOJ MPM｜影響面：利率/匯率｜source=BOJ", global_lines)
        self.assertIn("06/15-17 G7 Evian｜影響面：政治風險｜source=G7", global_lines)
        self.assertIn("06/16-17 Fed FOMC SEP｜影響面：利率/匯率｜source=Fed", global_lines)
        self.assertNotIn("全球事件：source-error", watch)
    ...
    def test_summary_includes_strategy_evidence_without_changing_actions(self):
        ...
        self.assertIn("v20.4.46 簡報", evidence)
        self.assertNotIn("資料依據", evidence)
    ...
    def test_v19_3_2_intraday_summary_classifies_0526_cases(self):
        ...
        self.assertIn("v20.4.46", summary_message(messages))
    ...
    def test_v20_4_cross_day_repair_promotes_prepare_without_buying(self):
        ...
        summary = summary_message(messages)
        unheld = unheld_message(messages)
        self.assertIn("【05/29 盤中｜v20.4.46】", summary)
    ...
    def test_post_market_add_plan_is_not_labeled_as_risk_control(self):
        ...
        summary = summary_message(messages)
        self.assertIn("【05/28 盤後｜v20.4.46】", summary)
    ...
    def test_holiday_second_take_profit_uses_latest_trading_day_execution_memory(self):
        ...
        self.assertEqual(generator.position_summary_action("英業達", payload), "第二段停利後觀察")
        self.assertIn("【05/31 假日｜v20.4.46】", summary)
        self.assertIn("【英業達 2356】📌 第二段停利後觀察", card)
    ...
    def test_intraday_v20_0_10_execution_contract(self):
        ...
        self.assertIn("【05/28 盤中｜v20.4.46】", summary)
    ...
    def test_v20_2_5_post_market_noise_contract_with_prepare_only_tracking(self):
        ...
        self.assertIn("【05/29 盤後｜v20.4.46】", summary)
        self.assertIn("📌 盤後簡報", summary)
        self.assertIn("結論：今日無有效新倉；既有持倉以收盤後風控觀察為主。", summary)
    ...
    def test_v20_4_39_post_market_mixed_trend_and_prepare_keeps_trend_actionable(self):
        ...
        self.assertIn("【06/03 盤後｜v20.4.46】", summary)
        self.assertIn("趨勢延續買入 1 檔小倉", summary)
    ...
    def test_v20_4_12_complete_message_list_orders_holdings_unheld_summary_evidence_details(self):
        ...
        self.assertIn("【06/01 盤中｜v20.4.46】", messages[0])
        self.assertIn("【06/01 盤中｜v20.4.46】", messages[1])
        self.assertIn("【06/01 盤中｜v20.4.46】", messages[2])
        ...
        self.assertIn("🧾 v20.4.46 簡報", messages[2])
    ...
    def test_v20_4_10_complete_report_context_manifest_maps_visible_fields(self):
        ...
        self.assertEqual(context["report_context"]["version"], "v20.4.46")
        ...
        self.assertIn("🧾 v20.4.46 簡報", summary_message(messages))
    ...
    def test_v20_4_10_manifest_fails_closed_for_missing_candidate_sources(self):
        ...
        self.assertIn("【05/31 盤中｜v20.4.46】", summary)
    ...
    def test_0603_v20_4_32_failure_specimen_message_list_replay(self):
        ...
        self.assertIn("【06/03 盤中｜v20.4.46】", summary)
    ...
    def test_report_score_evidence_display_owner_specimen_message_list_replay(self):
        ...
        self.assertIn("【06/03 盤後｜v20.4.46】", rendered)
    ...
    def test_eight_day_confirmed_market_theme_is_decision_eligible(self):
        ...
        self.assertEqual(generator.VERSION, "v20.4.46")
    ...
    def test_per_stock_backtest_context_drives_strategy_ready_when_global_sample_partial(self):
        ...
        rendered = "\n\n".join(messages)
        self.assertIn("【06/03 盤中｜v20.4.46】", rendered)
    ...
    def test_v20_4_42_postmarket_unheld_gate_attribution_readability_message_list_replay(self):
        ...
        self.assertIn("【06/04 盤後｜v20.4.46】", summary)
        self.assertIn("新倉建議 1", summary)
    ...
    def test_v20_4_43_evidence_chain_decision_judgments_cover_eligibility_layers(self):
        ...
        self.assertIn("【06/04 盤中｜v20.4.46】", summary_message(messages))
