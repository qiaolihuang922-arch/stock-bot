...
def evidence_message(messages):
    return next(message for message in messages if "簡短證據摘要" in message)
...
class GeneratorReportTest(unittest.TestCase):
    def test_generate_report_fails_closed_when_holdings_source_missing(self):
        ...
        self.assertIn("｜v20.4.13】", summary)
    ...
    def test_v19_3_2_intraday_summary_classifies_0526_cases(self):
        ...
        self.assertIn("v20.4.13", summary_message(messages))
    ...
    def test_v20_4_cross_day_repair_promotes_prepare_without_buying(self):
        ...
        self.assertIn("【05/29 盤中｜v20.4.13】", summary)
    ...
    def test_post_market_add_plan_is_not_labeled_as_risk_control(self):
        ...
        summary = summary_message(messages)
        self.assertIn("【05/28 盤後｜v20.4.13】", summary)
    ...
    def test_intraday_v20_0_10_execution_contract(self):
        ...
        self.assertIn("【05/28 盤中｜v20.4.13】", summary)
    ...
    def test_v20_2_5_post_market_noise_contract_with_prepare_only_tracking(self):
        ...
        self.assertIn("【05/29 盤後｜v20.4.13】", summary)
        self.assertIn("今日交易\n新增交易建議：無", summary)
        self.assertIn("已執行（不重複下單）", summary)
    ...
    def test_v20_4_12_complete_message_list_orders_holdings_unheld_summary_evidence_details(self):
        ...
        self.assertIn("【06/01 盤中｜v20.4.13】", messages[0])
        self.assertIn("【06/01 盤中｜v20.4.13】", messages[1])
        self.assertIn("【06/01 盤中｜v20.4.13】", messages[2])
        ...
        self.assertIn("v20.4.13 簡短證據摘要", messages[2])
        self.assertIn("本次證據摘要：", messages[2])
        self.assertIn("持倉判斷依據：已讀取可驗證的持久化持倉紀錄", messages[2])
        self.assertIn("未持倉判斷依據：候選來源可讀，但今日策略樣本不足", messages[2])
        self.assertIn("結論：新倉無有效進場；持倉依風控清單處理。", messages[2])
    ...
    def test_v20_4_12_2356_still_holding_without_confirmed_second_tp_fails_closed(self):
        ...
        rendered = "\n\n".join(messages)
        self.assertEqual(generator.position_summary_action("英業達", payload), "停利記憶不足")
        self.assertIn("英業達 缺少可驗證的第二段停利執行紀錄", evidence_message(messages))
        self.assertIn("維持 fail-closed 風控", evidence_message(messages))
        for term in FORBIDDEN_SHORT_EVIDENCE_TERMS:
    ...
    def test_v20_4_13_noise_reduction_moves_verbose_detail_after_short_evidence(self):
        ...
        self.assertIn("今日策略樣本與候選來源不足", evidence_message(messages))
        self.assertIn("資料不足處理：策略樣本、候選清單不足時採 fail-closed", evidence_message(messages))
        self.assertIn("結論：新倉無有效進場", evidence_message(messages))
        for term in FORBIDDEN_SHORT_EVIDENCE_TERMS:
            self.assertNotIn(term, evidence_message(messages))
    ...
    def test_v20_4_13_unavailable_strategy_sample_short_evidence_fails_closed_naturally(self):
        ...
        self.assertIn("今日策略樣本與候選來源不足", evidence)
        self.assertIn("無法支持可行動新倉結論", evidence)
        self.assertIn("結論：新倉無有效進場", evidence)
    ...
    def test_v20_4_10_complete_report_context_manifest_maps_visible_fields(self):
        ...
        self.assertEqual(context["report_context"]["version"], "v20.4.13")
        ...
        self.assertIn("持倉判斷依據：已讀取可驗證的持久化持倉紀錄", evidence_message(messages))
    ...
    def test_v20_4_10_manifest_fails_closed_for_missing_candidate_sources(self):
        ...
        self.assertIn("【05/31 盤中｜v20.4.13】", summary)
