...
class GeneratorReportTest(unittest.TestCase):
    ...
    def test_v20_4_43_trend_continuation_rr_hard_gate_fails_closed_official_report(self):
        ...
        self.assertIn("不能買：風險報酬還不夠", card)
        self.assertIn("還差：風險報酬 0.8→1.5（差0.7）", card)
        self.assertIn("可買條件：風險報酬 >= 1.5", card)
    ...
    def test_telegram_messages_use_summary_cards_and_detail(self):
        ...
        self.assertIn("買點：不買，等風險報酬達標", unheld_message(messages))
    ...
    def test_rejected_weak_rr_uses_true_reject_reason_not_rr(self):
        ...
        self.assertIn("買點：不可買，等結構修復", unheld_message(messages))
    ...
    def test_v21_1_strong_rebound_uses_multi_window_retest_context(self):
        ...
        self.assertIn("還差：急彈後先等回測；回測區 118~118.59不破；量能偏弱（V10 0.44x / V20 0.62x）；品質 D→B以上；理論風險報酬 1.6僅參考", card)
        self.assertIn("可買條件：回測區 118~118.59不破 + 非漲停追價 + 量能有效 + 品質B以上 + 風險報酬>=1.5", card)
        self.assertIn("盤中觸發：回測不破且非漲停追價", card)
        self.assertIn("數據：理論風險報酬 1.6", card)
    ...
    def test_v20_0_14_post_market_fixture_uses_next_day_plan_semantics(self):
        ...
        self.assertIn("不能買：盤後訊號需開盤確認", unheld)
        self.assertIn("還差：盤後待開盤確認", unheld)
        self.assertIn("可買條件：開盤後守突破區 + 不追價", unheld)
    ...
    def test_v20_4_39_post_market_mixed_trend_and_prepare_keeps_trend_actionable(self):
        ...
        self.assertIn("不能買：盤後訊號需開盤確認", ordinary_card)
        self.assertIn("還差：盤後待開盤確認", ordinary_card)
        self.assertIn("可買條件：開盤後守突破區 + 不追價", ordinary_card)
    ...
    def test_afterhours_cards_are_denoised_without_first_read_preface(self):
        ...
        self.assertNotIn("\n數據：", holding_card)
        self.assertNotIn("\n盤面：", unheld_card)
        self.assertIn("買點：不可買", unheld_card)
        self.assertIn("不能買：", unheld_card)
        self.assertIn("還差：", unheld_card)
    ...
    def test_v20_4_21_afterhours_mobile_readability_probe(self):
        ...
        self.assertNotRegex(position, r"數據：風險報酬 2\.73")
        self.assertIn("不能買：短線過熱，先等冷卻", unheld)
        self.assertIn("還差：熱度 Lv.2；降到 Lv.1/觀察以下", unheld)
    ...
    def test_v20_4_10_mixed_valid_and_source_ineligible_buy_like_stays_consistent(self):
        ...
        self.assertIn("不能買：資料來源缺失", invalid_card)
        self.assertIn("還差：補齊行情/策略來源", invalid_card)
    ...
    def test_v20_4_25_strategy_sample_source_error_blocks_action_without_hiding_available_price(self):
        ...
        self.assertIn("不能買：策略樣本不足", card)
        self.assertIn("還差：補足有效策略樣本", card)
    ...
    def test_report_score_evidence_display_owner_specimen_message_list_replay(self):
        ...
        self.assertIn("不能買：短線過熱，先等冷卻", overheat_card)
        self.assertIn("還差：熱度 Lv.2；降到 Lv.1/觀察以下", overheat_card)
    ...
    def test_v20_4_42_postmarket_unheld_gate_attribution_readability_message_list_replay(self):
        ...
        self.assertIn("不能買：風險報酬還不夠", rr_near_card)
        self.assertIn("還差：風險報酬 0.99→1.5（差0.51）", rr_near_card)
        self.assertNotIn("距突破 2%", rr_near_card)
        self.assertNotIn("盤後待確認", rr_near_card)
        self.assertIn("【技嘉 2376】⏳ 等冷卻｜過熱觀察", hot_card)
        self.assertIn("不能買：短線過熱，先等冷卻", hot_card)
        self.assertIn("還差：熱度 Lv.2；降到 Lv.1/觀察以下", hot_card)
        self.assertIn("可買條件：降到 Lv.1/觀察以下 + 回測不破", hot_card)
        self.assertNotIn("風險報酬 1.4｜需>=1.5", hot_card)
        self.assertNotIn("進場品質", hot_card)
        self.assertIn("【華邦電 2344】⏳ 等冷卻｜過熱 Lv.3", extreme_card)
        self.assertIn("不能買：漲停/過熱，不追價", extreme_card)
        self.assertIn("還差：熱度 Lv.3；降到 Lv.1/觀察以下", extreme_card)
        self.assertIn("可買條件：降到 Lv.1/觀察以下 + 回測不破 + 非漲停追價", extreme_card)
        ...
        self.assertIn("不能買：尚未站回突破區", failed_card)
        self.assertIn("還差：距突破區 9%；買點區<=5%；差4%", failed_card)
        self.assertIn("可買條件：重新站回突破區後再評估", failed_card)
        ...
        self.assertIn("依據：理論風險報酬僅參考；量能達標；回測僅輔助", prepare_card)
        ...
        self.assertNotIn("不能買：", buyable_card)
        self.assertNotIn("還差：", buyable_card)
        self.assertNotIn("距突破 0.5%/需<=4%", buyable_card)
        self.assertNotIn("買點距離已觸發/不追價", buyable_card)
        self.assertNotIn("不能買：", trend_card)
        self.assertNotIn("還差：", trend_card)
        self.assertNotIn("不能買：", actionable_trend_card)
        self.assertNotIn("還差：", actionable_trend_card)
        self.assertNotIn("買點距離已觸發/不追價", actionable_trend_card)
        self.assertIn("不能買：漲跌停鎖定，不追價", limit_card)
        self.assertIn("還差：解除鎖定後再評估", limit_card)
        self.assertIn("不能買：反彈還沒轉強", weak_card)
        self.assertIn("還差：放量轉強後再評估", weak_card)
    ...
    def test_confirmed_evidence_preserves_limit_lock_chase_hard_blocker(self):
        ...
        self.assertIn("買點：不買，等回測", card)
        self.assertIn("不能買：漲跌停鎖定，不追價", card)
        self.assertIn("還差：解除鎖定後再評估", card)
