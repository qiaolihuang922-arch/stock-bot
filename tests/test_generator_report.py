...
class GeneratorReportTest(unittest.TestCase):
    ...
    def test_v20_4_38_rr_wait_card_reason_and_backtest_summary_readability(self):
        ...
        self.assertIn("數據：風險報酬 0.98｜不適用（風險報酬不足）｜原因：風險報酬不足，等待修復｜V 0.86x", liteon_card)
    ...
    def test_v21_1_strong_rebound_uses_multi_window_retest_context(self):
        ...
        self.assertIn("缺口：急彈後先等回測；回測區 118~118.59不破；量能偏弱（10日量 0.44x / 20日量 0.62x）；買點品質：回測 / 轉強後重評；潛在報酬：好（1.6倍），但尚未回測確認", card)
        self.assertIn("可買：回測區 118~118.59不破 + 非漲停追價 + 量能有效 + 買點品質 B 以上 + 風險報酬 >= 1.5", card)
        self.assertNotIn("\n狀態：", card)
        self.assertNotIn("\n進場檢查：", card)
        self.assertNotIn("\n拆解：", card)
        self.assertNotIn("\n買點：不買，等回測", card)
    ...
    def test_v21_1_retest_anchor_says_breakout_zone_when_price_is_below_zone(self):
        ...
        self.assertIn("可買：先站回突破區 118~118.59，再回測不破 + 非漲停追價 + 量能有效 + 買點品質 B 以上 + 風險報酬 >= 1.5", card)
    ...
    def test_v19_3_2_intraday_summary_classifies_0526_cases(self):
        ...
        self.assertIn("風險報酬 -（過熱）", unheld_message(messages))
    ...
    def test_low_volume_limit_up_prepare_card_and_summary_show_risk(self):
        ...
        self.assertIn("數據：風險報酬 -（過熱）｜不適用（過熱）｜證據：過熱不適用｜V 0.62x", low_volume_card)
    ...
    def test_v20_4_21_afterhours_mobile_readability_probe(self):
        ...
        self.assertNotRegex(position, r"數據：風險報酬 2\.73")
        self.assertIn("進場：不買，等冷卻｜原因：短線過熱，先等冷卻", unheld)
        self.assertIn("缺口：熱度 Lv.2；降到 Lv.1/觀察以下", unheld)
    ...
    def test_0603_v20_4_32_failure_specimen_message_list_replay(self):
        ...
        self.assertIn("數據：風險報酬 -（過熱）", gigabyte_card)
    ...
    def test_report_score_evidence_display_owner_specimen_message_list_replay(self):
        ...
        self.assertIn("缺口：熱度 Lv.2；降到 Lv.1/觀察以下", overheat_card)
    ...
    def test_v20_4_42_postmarket_unheld_gate_attribution_readability_message_list_replay(self):
        ...
        self.assertIn(
            "缺口：風險報酬 0.98→1.5（差0.52）；距突破 6%；買點區<=5%；差1%；另等趨勢延續/回測承接買點型態",
            rr_card,
        )
        ...
        self.assertIn("缺口：熱度 Lv.2；降到 Lv.1/觀察以下", hot_card)
        ...
        self.assertIn("缺口：熱度 Lv.3；降到 Lv.1/觀察以下", extreme_card)
        ...
        self.assertIn("缺口：距突破區 9%；買點區<=5%；差4%", failed_card)
    ...
    def test_confirmed_evidence_preserves_limit_lock_chase_hard_blocker(self):
        ...
        self.assertIn("風險報酬 -（過熱）｜不適用（過熱）｜證據：過熱不適用", card)
