...
class GeneratorReportTest(unittest.TestCase):
    ...
    def test_position_cards_follow_summary_order_and_decision_wording(self):
        ...
        self.assertIn("下一步：修復才恢復優先級，未修復續降級", position_msg)
    ...
    def test_v21_1_strong_rebound_uses_multi_window_retest_context(self):
        ...
        self.assertIn("進場：不買，等回測｜原因：急彈後還沒回測確認", card)
        self.assertIn("缺口：回測區 118~118.59不破", card)
        self.assertIn("可買：回測區 118~118.59不破 + 非漲停追價 + 量能有效", card)
    ...
    def test_v21_1_multi_day_weak_rebound_repairs_from_rejected_to_retest_wait(self):
        ...
        self.assertIn("缺口：等待回測最近反彈收盤 159 附近不破", card)
        self.assertIn("可買：回測最近反彈收盤 159 附近不破 + 非追高 + 量能有效", card)
    ...
    def test_v21_1_retest_anchor_says_breakout_zone_when_price_is_below_zone(self):
        ...
        self.assertIn("缺口：站回突破區 118~118.59", card)
        self.assertIn("可買：先站回突破區 118~118.59，再回測不破 + 非漲停追價 + 量能有效", card)
    ...
    def test_unheld_overheat_prepare_rr_value_uses_overheat_blocker(self):
        ...
        self.assertIn("進場：不買，等冷卻｜原因：短線過熱，先等冷卻", card)
    ...
    def test_v20_2_4_r3_hot_market_adds_prepare_layer_without_buying(self):
        ...
        self.assertIn("進場：不買，等回測｜原因：漲停/過熱，不追價", unheld)
    ...
    def test_low_volume_limit_up_prepare_card_and_summary_show_risk(self):
        ...
        self.assertIn("進場：不買，等回測｜原因：漲停/過熱，不追價", low_volume_card)
    ...
    def test_afterhours_cards_are_denoised_without_first_read_preface(self):
        ...
        self.assertIn("下一步：", holding_card)
        self.assertNotIn("\n條件：", holding_card)
        self.assertNotIn("\n數據：", holding_card)
        self.assertNotIn("\n盤面：", unheld_card)
    ...
    def test_telegram_message_noise_consistency_intraday_owner_nine_points(self):
        ...
        self.assertNotIn("回測：不可用", unheld)
        self.assertNotIn("樣本不足（有效樣本3）", unheld)
        self.assertIn("缺口：熱度 Lv.2", partial_card)
    ...
    def test_0603_v20_4_32_failure_specimen_message_list_replay(self):
        ...
        self.assertIn("缺口：熱度 Lv.2", gigabyte_card)
    ...
    def test_v20_4_42_postmarket_unheld_gate_attribution_readability_message_list_replay(self):
        ...
        self.assertIn("進場：不買，等冷卻｜原因：漲停/過熱，不追價", extreme_card)
        self.assertIn("缺口：熱度 Lv.3", extreme_card)
        self.assertIn("可買：降到 Lv.1/觀察以下 + 回測不破 + 非漲停追價", extreme_card)
        ...
        self.assertIn("缺口：等待回測最近反彈收盤 51.4 附近不破", weak_card)
        self.assertIn("可買：回測最近反彈收盤 51.4 附近不破 + 非追高 + 量能有效", weak_card)
    ...
    def test_confirmed_evidence_preserves_limit_lock_chase_hard_blocker(self):
        ...
        self.assertIn("進場：不買，等回測｜原因：漲跌停鎖定，不追價", card)
        self.assertIn("缺口：解除鎖定後再評估", card)
