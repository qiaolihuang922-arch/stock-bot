...
class GeneratorReportTest(unittest.TestCase):
    def test_generate_report_fails_closed_when_holdings_source_missing(self):
        ...
        self.assertIn("｜v20.4.19】", summary)
        self.assertIn("🧾 v20.4.19 簡報＋資料依據", summary)
        ...
        self.assertIn("缺資料標的本輪不給進場結論", detail)
    ...
    def test_summary_includes_strategy_evidence_without_changing_actions(self):
        ...
        self.assertIn("v20.4.19 簡報＋資料依據", evidence)
    ...
    def test_v19_3_2_intraday_summary_classifies_0526_cases(self):
        ...
        self.assertIn("v20.4.19", summary_message(messages))
    ...
    def test_v20_4_cross_day_repair_promotes_prepare_without_buying(self):
        ...
        self.assertIn("【05/29 盤中｜v20.4.19】", summary)
    ...
    def test_post_market_add_plan_is_not_labeled_as_risk_control(self):
        ...
        summary = summary_message(messages)
        self.assertIn("【05/28 盤後｜v20.4.19】", summary)
    ...
    def test_holiday_second_take_profit_uses_latest_trading_day_execution_memory(self):
        ...
        self.assertEqual(generator.position_summary_action("英業達", payload), "第二段停利後觀察")
        self.assertIn("【05/31 假日｜v20.4.19】", summary)
        self.assertIn("【英業達 2356】📌 第二段停利後觀察", card)
        ...
        self.assertIn("候選資料可支持分類", summary)
    ...
    def test_intraday_v20_0_10_execution_contract(self):
        ...
        self.assertIn("【05/28 盤中｜v20.4.19】", summary)
    ...
    def test_intraday_v20_0_12_separates_mainline_from_execution(self):
        ...
        self.assertIn("【05/28 盤中｜v20.4.19】", summary)
        self.assertIn("市場 / 題材背景：近幾個交易證據日不足以形成可靠背景，只作觀察，不作買點。", summary)
    ...
    def test_v20_2_5_post_market_noise_contract_with_prepare_only_tracking(self):
        ...
        self.assertIn("【05/29 盤後｜v20.4.19】", summary)
        self.assertIn("今日交易\n新增交易建議：無", summary)
        self.assertIn("已執行（不重複下單）", summary)
    ...
    def test_v20_4_12_complete_message_list_orders_holdings_unheld_summary_evidence_details(self):
        ...
        self.assertIn("【06/01 盤中｜v20.4.19】", messages[0])
        self.assertIn("【06/01 盤中｜v20.4.19】", messages[1])
        self.assertIn("【06/01 盤中｜v20.4.19】", messages[2])
        ...
        self.assertIn("🧾 v20.4.19 簡報＋資料依據", messages[2])
    ...
    def test_v20_4_12_2356_still_holding_without_confirmed_second_tp_fails_closed(self):
        ...
        self.assertIn("候選資料可支持分類", evidence_message(messages))
        for term in FORBIDDEN_SHORT_EVIDENCE_TERMS:
    ...
    def test_v20_4_10_complete_report_context_manifest_maps_visible_fields(self):
        ...
        self.assertEqual(context["report_context"]["version"], "v20.4.19")
        ...
        self.assertIn("🧾 v20.4.19 簡報＋資料依據", summary_message(messages))
        ...
        self.assertIn("市場 / 題材背景：近 20 個交易證據日，近期 5 日支持仍支持目前背景觀察", evidence_message(messages))
        self.assertIn("策略樣本：本輪缺少可驗證樣本，可靠度低，未納入買賣判斷。", evidence_message(messages))
        self.assertIn("市場題材:\nsource: production-market-theme\nstatus: available", evidence_message(messages))
        self.assertIn("策略樣本:\nsource: missing-source\nstatus: missing-source", evidence_message(messages))
        self.assertIn("conflict: none", evidence_message(messages))
        for term in FORBIDDEN_SHORT_EVIDENCE_TERMS:
            self.assertNotIn(term, evidence_message(messages))
        ...
        for case in [
            ...
            with self.subTest(case=case):
                ...
                if case == "missing_strategy_sample_source":
                    self.assertIn("status: missing-source", rendered)
                    self.assertIn("新倉：無有效進場。", rendered)
                    self.assertNotIn("建準｜可買", rendered)
                    self.assertNotIn("買點：可買", rendered)
                if case == "ledger_position_conflict":
                    self.assertIn("status: unresolved-conflict", rendered)
                    self.assertIn("conflict: position-vs-event", rendered)
    ...
    def test_v20_4_19_maturity_report_scores_100_for_readonly_fail_closed_cases(self):
        for case in [
            ...
            with self.subTest(case=case):
                ...
                self.assertEqual(report["generator_version"], "v20.4.19")
                ...
                for term in ["source:", "status:", "use:", "limit:", "conflict:"]:
                    self.assertIn(term, report["telegram_messages"][2])
    ...
    def test_v20_4_19_maturity_report_blocks_synthetic_strategy_and_stale_runner(self):
    ...
    def test_v20_4_10_manifest_fails_closed_for_missing_candidate_sources(self):
        ...
        self.assertIn("【05/31 盤中｜v20.4.19】", summary)
