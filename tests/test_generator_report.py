...
from core.future_watch import (
    ...
    build_live_stock_fundamentals_source,
...
class GeneratorReportTest(unittest.TestCase):
    def test_v21_0_6_trading_day_before_open_is_pre_market_not_non_trading(self):
        class FixedDateTime(datetime):
            current = datetime(2026, 6, 15, 8, 0)
        ...
        with patch.object(generator, "datetime", FixedDateTime):
            self.assertEqual(generator.get_market_phase(), "盤前")

        FixedDateTime.current = datetime(2026, 6, 15, 10, 0)
        with patch.object(generator, "datetime", FixedDateTime):
            self.assertEqual(generator.get_market_phase(), "盤中")

        FixedDateTime.current = datetime(2026, 6, 14, 8, 0)
        with patch.object(generator, "datetime", FixedDateTime):
            self.assertEqual(generator.get_market_phase(), "假日")

    def test_v21_0_6_pre_market_summary_uses_today_plan_not_tomorrow_plan(self):
        payload = self.breakout_distance_payload(2, holding=True)
        ...
        self.assertIn("【06/15 盤前｜v21.1】", summary)
        self.assertNotIn("06/15 非交易", summary)
        self.assertNotIn("明日計畫", summary)
    ...
    def test_v20_4_38_rr_wait_card_reason_and_backtest_summary_readability(self):
        ...
        self.assertIn("【光寶科 2301】👀 等風險報酬｜風險報酬不足", liteon_card)
        self.assertIn("缺口：風險報酬 0.98→1.5（差0.52）", liteon_card)
        self.assertNotIn("數據：風險報酬 0.98", liteon_card)
        ...
        self.assertIn("僅追蹤 1（等風險報酬）", summary)
    ...
    def test_trend_continuation_official_report_has_separate_small_buy_bucket(self):
        ...
        self.assertIn("未持倉 1（趨勢延續1）", evidence)
    ...
    def test_v20_4_43_trend_continuation_rr_hard_gate_fails_closed_official_report(self):
        ...
        self.assertNotIn("未持倉 1（趨勢延續1）", evidence)
        self.assertNotIn("【智原 3231】🟢 趨勢延續買入｜小倉", rendered)
        self.assertNotIn("買點：趨勢延續買入｜小倉 <=15%", card)
        self.assertIn("【智原 3231】👀 等風險報酬｜風險報酬不足", card)
        self.assertIn("進場：不買，等風險報酬達標｜原因：風險報酬還不夠", card)
        self.assertIn("缺口：風險報酬 0.8→1.5（差0.7）", card)
        self.assertIn("可買：風險報酬 >= 1.5", card)
    ...
    def test_score_source_error_unheld_card_hides_s_number_without_hiding_price_or_rr(self):
        ...
        self.assertIn("數據：風險報酬 2.1｜S 不可用｜V 1.5x", card)
        self.assertIn("盤面：強弱證據不足｜待確認", card)
        self.assertIn("價格：100.0（+1.20%）", card)
        self.assertNotIn("S 5/5", card)
        self.assertNotIn("突破確認", card)
    ...
    def test_holding_add_uses_basis_label(self):
        ...
        self.assertIn("├─ 依據：小幅轉強、風險報酬達標、信心達標", report)
    ...
    def test_telegram_messages_use_summary_cards_and_detail(self):
        ...
        self.assertIn("今日盤中風控建議", summary_message(messages))
        self.assertIn("智原 加碼10（加碼後守警戒價，量價未延續則停止加碼）", summary_message(messages))
        self.assertIn("未持倉 1 檔僅追蹤，等觸發，不列入今日盤中風控建議", summary_message(messages))
        self.assertNotIn("建準｜等風險報酬｜不追價，等風險報酬達標", summary_message(messages))
        self.assertIn("未持倉狀態：", summary_message(messages))
        self.assertIn("未持倉 1｜僅追蹤 1（等風險報酬）", summary_message(messages))
        self.assertNotIn("不可追高觀察 0（不可買）", summary_message(messages))
        self.assertIn("僅追蹤 1（等風險報酬）", summary_message(messages))
    ...
    def test_v20_4_47_future_30d_watch_optional_fourth_message_official_list(self):
        ...
        self.assertIn(
            "06/20 2301 光寶科｜J.P. Morgan Asia Pacific All Star Forum - London｜關注原因：持倉",
            messages[3],
        )
        ...
        self.assertNotIn("2301 光寶科｜EPS", messages[3])
    ...
    def test_v20_4_47_live_twse_source_builds_pressure_timeline(self):
        ...
        self.assertEqual(len(TAIWAN_CRASH_TEMPLATE_LIBRARY), 19)
        self.assertIn("2015 台股急跌/中國股災外溢", source["line"])
        self.assertIn("相似度 70%", source["line"])
        self.assertIn("型態 高檔壓力前段", source["line"])
        self.assertIn("相似點：跌幅 -1.68%｜高檔回落 -1.88%｜盤中震盪 +1.50%｜5日位置 48%", source["line"])
        self.assertIn("模組分數：價格", source["line"])
        self.assertIn("不相似/限制：屬壓力前段，不是崩盤等級", source["line"])
        self.assertIn("下一步觀察：隔日是否續破當日低點", source["line"])
        self.assertIn("資料：TWSE近4日｜樣本庫台股急跌 19件", source["line"])
    ...
    def test_v21_0_1_twse_historical_source_retries_transient_api_error(self):
        calls = {"MI_INDEX": 0, "MI_5MINS_HIST": 0}
        responses = {
            "MI_INDEX": [
                {"指數": "發行量加權股價指數", "收盤指數": "22600.00", "漲跌百分比": "-1.20", "成交金額": "3000000"},
            ],
            "MI_5MINS_HIST": [
                {"Date": "1150608", "OpeningIndex": "22600.00", "HighestIndex": "23000.00", "LowestIndex": "22500.00", "ClosingIndex": "22900.00", "TradingValue": "2500000"},
                {"Date": "1150609", "OpeningIndex": "22900.00", "HighestIndex": "23100.00", "LowestIndex": "22600.00", "ClosingIndex": "22800.00", "TradingValue": "2600000"},
                {"Date": "1150610", "OpeningIndex": "22800.00", "HighestIndex": "22900.00", "LowestIndex": "22400.00", "ClosingIndex": "22600.00", "TradingValue": "3000000"},
            ],
        }

        def flaky_get_json(url, **_kwargs):
            key = "MI_INDEX" if "MI_INDEX" in url else "MI_5MINS_HIST"
            calls[key] += 1
            if key == "MI_INDEX" and calls[key] == 1:
                raise RuntimeError("temporary TWSE timeout")
            return SimpleNamespace(json=lambda: responses[key], raise_for_status=lambda: None)

        source = build_live_twse_historical_source(datetime(2026, 6, 10), get_json=flaky_get_json)

        self.assertEqual(source["status"], "available")
        self.assertGreaterEqual(calls["MI_INDEX"], 2)
        self.assertIn("source=TWSE", source["line"])
        self.assertNotIn("source-error", source["line"])

    def test_v21_0_1_twse_historical_source_error_line_is_not_false_no_similarity(self):
        def broken_get_json(_url, **_kwargs):
            raise RuntimeError("TWSE unavailable")

        source = build_live_twse_historical_source(datetime(2026, 6, 10), get_json=broken_get_json)

        self.assertEqual(source["status"], "source-error")
        self.assertIn("TWSE 官方來源暫時不可讀", source["line"])
        self.assertIn("source=TWSE source-error", source["line"])
        self.assertNotIn("無高相似台股急跌樣本", source["line"])

    def test_v21_0_1_stock_fundamentals_loads_twse_revenue_openapi(self):
        def fake_get_json(url, **_kwargs):
        ...
        source = build_live_stock_fundamentals_source(datetime(2026, 6, 10), get_json=fake_get_json)
        item = source["items_by_code"]["2303"]

        self.assertEqual(source["status"], "available")
        self.assertEqual(item["eps"], "1.29")
        self.assertEqual(item["revenue_month"], "11505")
        self.assertEqual(item["revenue_yoy"], "17.8")
    ...
    def test_v20_4_47_live_twse_source_uses_severe_taiwan_crash_template(self):
        ...
        self.assertIn("型態 槓桿平倉急殺", source["line"])
        self.assertIn("壓力級別 急跌", source["line"])
        self.assertIn("相似點：跌幅 -8.35%｜高檔回落 -11.47%", source["line"])
        self.assertIn("模組分數：價格", source["line"])
        self.assertIn("資料：TWSE近3日｜樣本庫台股急跌 19件", source["line"])
        self.assertNotIn("全球股災", source["line"])
        ...
        def future_sources(now):
            return {
                "historical_source": {
                    ...
                    "line": (
                        "歷史類比：2015 台股急跌/中國股災外溢｜相似度 70%｜型態 高檔壓力前段｜壓力級別 壓力前段｜source=TWSE\n"
                        "相似點：跌幅 -1.68%｜高檔回落 -1.88%｜盤中震盪 +1.50%｜5日位置 48%\n"
                        "模組分數：價格 72｜位置 61｜量能 0｜情境 30\n"
                        "不相似/限制：屬壓力前段，不是崩盤等級；量能資料不足，未納入主要判斷\n"
                        "下一步觀察：隔日是否續破當日低點；未來3-5日是否跌破本月低點\n"
                        "資料：TWSE近4日｜樣本庫台股急跌 19件"
                    ),
                },
    ...
    def test_position_cards_follow_summary_order_and_decision_wording(self):
        ...
        self.assertIn("明日處理：修復才恢復優先級，未修復續降級", position_msg)
    ...
    def test_summary_with_holding_and_buy_has_no_zero_tracking_noise(self):
        ...
        self.assertIn("市場：中性觀察 R2｜執行動作 1（加碼10）｜新倉建議 1｜今日新建倉 0｜持倉風控 1｜未持倉 1（可買1）", summary_message(messages))
        self.assertIn("智原 加碼10（加碼後守警戒價，量價未延續則停止加碼）", summary_message(messages))
        self.assertIn("建準 可買（分批，不追價）", summary_message(messages))
        self.assertIn("未持倉 1｜可買 1", summary_message(messages))
    ...
    def test_unheld_funnel_prepare_count_has_separate_tracking_parent(self):
        ...
        self.assertIn("未持倉 4｜不可追高觀察 1（待確認）｜僅追蹤 3（等冷卻2/等風險報酬1）", text)
        ...
        self.assertNotIn("其中：等冷卻 2、等回測 0、等風險報酬 1、等量能 0", text)
    ...
    def test_summary_includes_strategy_evidence_without_changing_actions(self):
        ...
        self.assertIn("v21.1 簡報", evidence)
        self.assertNotIn("資料依據", evidence)
    ...
    def test_unheld_far_from_trigger_tracks_retest_instead_of_rejecting(self):
        ...
        self.assertIn("未持倉 1｜僅追蹤 1（等回測）", summary_message(messages))
    ...
    def test_rejected_weak_rr_uses_true_reject_reason_not_rr(self):
        ...
        self.assertIn("進場：不可買，等結構修復｜原因：反彈還沒轉強", unheld_message(messages))
        self.assertIn("盤中觸發：重新轉強前不列優先", unheld_message(messages))
        self.assertNotIn("淘汰：1 檔｜主因：風險報酬不足", summary_message(messages))
        self.assertNotIn("【旺宏 2337】⛔ 淘汰｜風險報酬不足", unheld_message(messages))
        self.assertNotIn("等風險報酬達標", unheld_message(messages))

    def test_v21_1_strong_rebound_uses_multi_window_retest_context(self):
        payload = render_payload(
        ...
        payload["stock_code"] = "2337"
        payload["volume_ratio"] = 0.44
        payload["volume_ratio_10"] = 0.44
        payload["volume_ratio_20"] = 0.62
        payload["retest_zone_low"] = 118
        payload["retest_zone_high"] = 118.59
        payload["result"].update({
            "decision": "WAIT",
            "price_behavior": "WEAK_REBOUND",
            "structure_phase": "WEAK_REBOUND",
        ...
        self.assertIn("狀態：急彈後還沒回測確認", card)
        self.assertIn("回測：回測區 118~118.59不破", card)
        self.assertIn("有效買點：不破 + 非漲停追價 + 量能有效", card)
        self.assertNotIn("買點品質：回測 / 轉強後重評", card)
        self.assertNotIn("潛在報酬：好（1.6倍），但尚未回測確認", card.split("盤中觸發：", 1)[0])
        self.assertIn("狀態：急彈後還沒回測確認", card)
        self.assertNotIn("\n進場檢查：", card)
        self.assertNotIn("\n拆解：", card)
        self.assertNotIn("\n買點：不買，等回測", card)
    ...
    def test_v21_1_retest_anchor_says_breakout_zone_when_price_is_below_zone(self):
        ...
        self.assertIn("回測：站回突破區 118~118.59", card)
        self.assertIn("有效買點：先站回突破區 118~118.59，再回測不破 + 非漲停追價 + 量能有效", card)
    ...
    def test_v19_3_2_intraday_summary_classifies_0526_cases(self):
        ...
        self.assertIn("v21.1", summary_message(messages))
        self.assertNotIn("📡 資料：即時價 realtime｜日線 yahoo", summary_message(messages))
        ...
        self.assertIn("等待：熱度 Lv.3", unheld_message(messages))
    ...
    def test_light_loss_shakeout_holding_displays_warning_even_when_decision_is_watch(self):
        ...
        self.assertIn("明日處理：若跌破停損或轉弱，優先風控", card)
    ...
    def test_unheld_overheat_prepare_rr_value_uses_overheat_blocker(self):
        ...
        self.assertIn("狀態：短線過熱，先等冷卻", card)
        self.assertIn("盤中觸發：降溫到 Lv.1 + 回測不破", card)
        self.assertNotIn("數據：風險報酬 -（過熱）", card)
        self.assertNotIn("風險報酬 0.21", card)
    ...
    def test_unheld_previous_eliminated_single_valid_entry_stays_conservative(self):
        ...
        self.assertIn("歷史：前次 eliminated", card)
    ...
    def test_v19_4_tracking_states_do_not_override_valid_buy_or_weak_reject(self):
        ...
        self.assertIn("建準 可買（分批，不追價）", summary_message(messages))
        self.assertIn("未持倉 2｜可買 1｜淘汰 1", summary_message(messages))
        self.assertNotIn("其中僅追蹤 0 檔拆分", summary_message(messages))
        self.assertIn("淘汰：1 檔｜主因：弱反彈待確認", summary_message(messages))
        self.assertNotIn("建準｜等", summary_message(messages))
    ...
    def test_v21_0_1_far_low_volume_weak_market_waits_approach_not_market_or_volume(self):
        ...
        self.assertIn("明日計畫：未持倉：緯創等接近。", summary_message(messages))
        self.assertNotIn("未持倉 1｜僅追蹤 1（等接近）", summary_message(messages))
        self.assertNotIn("淘汰 1", summary_message(messages))
        ...
        self.assertIn("進場：不買｜尚未接近突破區/回測支撐", unheld_message(messages))
        self.assertIn("等待：距突破 19.25%；有效買點只看：接近突破區 / 回測承接型態", unheld_message(messages))
        self.assertNotIn("可買：接近突破區/回測支撐", unheld_message(messages))
    ...
    def test_v19_4_backtest_changes_tracking_order_only(self):
        ...
        self.assertIn("未持倉 2 檔僅追蹤，等觸發，不列入今日盤中風控建議", summary_message(messages))
        self.assertIn("未持倉 2｜僅追蹤 2（等風險報酬）", summary_message(messages))
        ...
        self.assertIn("【仁寶 2324】👀 等風險報酬｜風險報酬不足", unheld_message(messages))
    ...
    def test_v19_3_3_holding_add_levels_display_explicit_add_actions(self):
        ...
        self.assertIn("明日處理：RR足夠，品質達標", card)
    ...
    def test_v20_4_cross_day_repair_promotes_prepare_without_buying(self):
        ...
        summary = summary_message(messages)
        unheld = unheld_message(messages)
        self.assertIn("【05/29 盤中｜v21.1】", summary)
        self.assertNotIn("僅追蹤：\n- 旺宏（修復中｜連續觀察 4 天，未達進場條件）", summary)
        self.assertNotIn("追蹤最強", summary)
        self.assertIn("未持倉 1｜不可追高觀察 1（待確認）", summary)
        self.assertIn("【旺宏 2337】👀 不可追高觀察｜突破回測", unheld)
    ...
    def test_v20_4_cross_day_reduce_history_does_not_suppress_hard_risk_reduce(self):
        ...
        self.assertIn("原因：風控升級，跌破警戒仍需補足降風險；風險依據", card)
        self.assertNotIn("條件：硬風控覆蓋，高於今日交易事件", card)
    ...
    def test_same_day_buy_entry_price_quick_stop_reduces_umc_fixture(self):
        ...
        summary = summary_message(messages)
        self.assertIn("【聯電 2303】📌 減碼", card)
        self.assertIn("原因：同日建倉後跌破快速止損 / 入場價 3%，觸發當日減碼", card)
        self.assertNotIn("條件：同日建倉後觸發快速止損 / 入場即錯", card)
    ...
    def test_same_day_buy_entry_candle_low_quick_stop_reduces(self):
        ...
        self.assertIn("原因：同日建倉後跌破快速止損 / 入場 K 棒低點，觸發當日減碼", position_message(messages))
    ...
    def test_post_market_add_plan_is_not_labeled_as_risk_control(self):
        ...
        summary = summary_message(messages)
        self.assertIn("【05/28 盤後｜v21.1】", summary)
        self.assertIn("📌 盤後簡報", summary)
        self.assertIn("結論：新倉無有效進場；持倉先看風控。", summary)
        self.assertIn("明日計畫：觀察持倉是否跌破警戒。", summary)
    ...
    def test_afterhours_brief_counts_today_buy_holdings_as_executed_new_positions(self):
        ...
        self.assertIn("📌 盤後簡報", summary)
        self.assertIn("結論：新倉無有效進場；今日買入紀錄守警戒觀察。", summary)
        self.assertIn("明日計畫：旺宏、光寶科、建準守警戒觀察。", summary)
    ...
    def test_afterhours_today_buy_holding_explains_current_non_buy_by_source(self):
        ...
        for source, expected_terms in cases:
            with self.subTest(source=source):
                ...
                self.assertIn("結論：新倉無有效進場；今日買入紀錄守警戒觀察。", brief)
                self.assertIn("明日計畫：光寶科守警戒觀察。", brief)
                self.assertNotIn("今日交易：已建立新倉 1 檔", brief)
    ...
    def test_post_market_holding_risk_precedes_tomorrow_plan_without_duplicate_downgrade(self):
        ...
        self.assertIn("📌 盤後簡報", summary)
        self.assertIn("結論：新倉無有效進場；今日買入紀錄守警戒觀察。", summary)
        self.assertIn("明日計畫：智原、緯創守警戒觀察。", summary)
        self.assertNotIn("今日無有效新倉", summary)
        self.assertNotIn("今日交易：已建立新倉", summary)
    ...
    def test_rejected_summary_shows_count_not_full_four_stock_names(self):
        ...
        self.assertIn("結論：新倉無有效進場；未持倉等觸發。", summary_msg)
        self.assertIn("明日計畫：未持倉僅追蹤，等觸發。", summary_msg)
    ...
    def test_post_reduce_watch_card_uses_sold_shares_fallback_without_sell_pct(self):
        ...
        self.assertIn("結論：新倉無有效進場；持倉先看風控。", summary_message(messages))
        self.assertIn("明日計畫：觀察持倉是否跌破警戒。", summary_message(messages))
    ...
    def test_v19_3_3_profit_reduce_stop_detail_lines_are_direct_actions(self):
        ...
        self.assertNotIn("條件：停損優先，避免虧損擴大", stop_card)
        self.assertIn("原因：跌破停損線，避免虧損擴大", stop_card)
        self.assertIn("明日處理：清出後不急回補，等重新出現買點", stop_card)
        ...
        self.assertNotIn("條件：高浮盈或過熱延伸，保留核心倉", profit_card)
        self.assertIn("原因：高浮盈且過熱延伸，先保留獲利", profit_card)
        self.assertIn("明日處理：保留核心倉，等待冷卻後再評估", profit_card)
    ...
    def test_v20_2_3_completed_same_day_second_take_profit_uses_db_execution_data(self):
        ...
        self.assertIn("明日處理：今日已執行，避免重複賣出", card)
    ...
    def test_intraday_v20_0_10_execution_contract(self):
        ...
        self.assertIn("【05/28 盤中｜v21.1】", summary)
        self.assertIn("今日盤中風控建議", summary)
        self.assertNotIn("明日執行", summary)
        self.assertIn("市場：中性觀察 R2｜執行動作 1（停利）｜新倉建議 1｜今日新建倉 0｜持倉風控 1｜未持倉 2（可買1/淘汰1）", summary)
        ...
        self.assertIn("進場：不可買，等結構修復｜原因：量能還沒補上", unheld)
    ...
    def test_intraday_v20_0_11_followup_review_contract(self):
        ...
        self.assertIn("未持倉 3｜僅追蹤 1（等風險報酬）｜淘汰 2", summary)
        self.assertIn("淘汰：2 檔｜主因：突破失敗", summary)
        self.assertIn("【群創 3481】⛔ 淘汰｜突破失敗", unheld)
        self.assertIn("原因：前次可買條件已失效：突破失敗或跌破進場條件｜補充：追價風險 / 過熱、風險報酬不可用，不作主因", unheld)
    ...
    def test_intraday_v20_0_12_separates_mainline_from_execution(self):
        ...
        self.assertIn("新倉：無有效進場", summary)
        self.assertIn("未持倉 2 檔僅追蹤，等觸發，不列入今日盤中風控建議", summary)
        self.assertNotIn("強勢準備：", summary)
        self.assertIn("未持倉 3｜僅追蹤 2（等冷卻1/等回測1）｜淘汰 1", summary)
        self.assertIn("僅追蹤 2（等冷卻1/等回測1）", summary)
        self.assertIn("淘汰：1 檔｜主因：突破失敗", summary)
        self.assertIn("【台積電 2330】📌 核心續抱", position)
        self.assertIn("決策：核心續抱，暫不加碼", position)
        self.assertIn("明日處理：跌破警戒價優先風控，等待冷卻", position)
        ...
        self.assertIn("狀態：尚未接近突破區", unheld)
        self.assertIn("【台達電 2308】⏳ 等冷卻｜過熱觀察", unheld)
        self.assertIn("狀態：短線過熱，先等冷卻", unheld)
        self.assertIn("【範例股 0000】⛔ 淘汰｜突破失敗", unheld)
        self.assertIn("進場：不可買，等重新轉強｜原因：尚未站回突破區", unheld)
    ...
    def test_intraday_v20_0_12_hot_market_without_ai_evidence_uses_neutral_mainline(self):
        ...
        self.assertIn("市場：進攻偏熱 R3｜執行動作 0｜今日新建倉 0｜持倉風控 0｜未持倉 2（僅追蹤2）", summary)
        self.assertIn("🧭 主線：市場偏多但買點未成立。", summary)
        self.assertIn("🧭 執行：新增買點未成立，先等回測，不追高。", summary)
    ...
    def test_v20_2_4_r3_hot_market_adds_prepare_layer_without_buying(self):
        ...
        self.assertIn("【05/29 盤中｜v21.1】", summary)
        self.assertIn("市場：進攻偏熱 R3｜執行動作 0｜今日新建倉 0｜持倉風控 0｜未持倉 5（不可追高觀察1/僅追蹤3/淘汰1）", summary)
        ...
        self.assertIn("未持倉 5｜不可追高觀察 1（待確認）｜僅追蹤 3（等冷卻1/等回測1/等風險報酬1）｜淘汰 1", summary)
        self.assertIn("僅追蹤 3（等冷卻1/等回測1/等風險報酬1）", summary)
        self.assertIn("【A 0001】⏳ 等回測｜漲停不追", unheld)
        self.assertIn("狀態：漲停/過熱，不追價", unheld)
        self.assertIn("【B 0002】⏳ 等冷卻｜過熱觀察", unheld)
        self.assertIn("狀態：短線過熱，先等冷卻", unheld)
        self.assertIn("【C 0003】👀 不可追高觀察｜突破回測", unheld)
        self.assertIn("進場：待觸發，不追高｜原因：資料來源缺失", unheld)
    ...
    def test_low_volume_limit_up_prepare_card_and_summary_show_risk(self):
        ...
        self.assertNotIn("\n數據：", low_volume_card)
        self.assertNotIn("風險報酬 -（過熱）", low_volume_card)
        self.assertIn("狀態：漲停/過熱，不追價", low_volume_card)
        self.assertIn("【緯創 3231】⏳ 等回測｜漲停不追", attack_volume_card)
        self.assertNotIn("\n數據：", attack_volume_card)
        self.assertNotIn("縮量漲停", attack_volume_card)
        self.assertNotIn("強勢準備：", summary)
        self.assertIn("未持倉 2｜僅追蹤 2（等回測）", summary)
    ...
    def test_v20_2_4_r3_hot_prepare_overflow_counts_hidden_statuses(self):
        ...
        self.assertIn("【05/29 盤中｜v21.1】", summary)
        self.assertIn("市場：進攻偏熱 R3｜執行動作 0｜今日新建倉 0｜持倉風控 0｜未持倉 8（不可追高觀察2/僅追蹤5/淘汰1）", summary)
        self.assertNotIn("市場/結論：進攻偏熱｜R3；R3 進攻偏熱", summary)
        self.assertIn(
            "強勢準備：\n"
            "- 突破回測：C 待觸發，不追高\n"
        ...
        self.assertIn("未持倉 8｜不可追高觀察 2（待確認）｜僅追蹤 5（等冷卻3/等回測1/等風險報酬1）｜淘汰 1", summary)
        ...
        self.assertIn("【G 0007】👀 等風險報酬｜風險報酬不足", unheld)
    ...
    def test_intraday_mobile_a1_unheld_non_actionable_not_primary_prepare(self):
        ...
        self.assertIn("新倉：無有效進場", summary)
        self.assertIn("未持倉 2｜僅追蹤 2（等冷卻1/等回測1）", summary)
    ...
    def test_v20_2_5_post_market_noise_contract_with_prepare_only_tracking(self):
        ...
        self.assertIn("【05/29 盤後｜v21.1】", summary)
        self.assertIn("📌 盤後簡報", summary)
        self.assertIn("結論：新倉無有效進場；持倉先看風控。", summary)
        self.assertIn("明日計畫：未持倉僅追蹤，等觸發。", summary)
        ...
        self.assertNotIn("等風險報酬 0", summary)
    ...
    def test_v20_0_14_message_list_uses_single_report_phase_when_phase_drifts(self):
        ...
        self.assertNotIn("今日盤中風控建議\n光寶科 可買（分批，不追價）", summary)
        self.assertIn("新倉建議\n光寶科 可買（分批，不追價）｜尚未買入｜建議分批｜盤中觸發", summary)
        self.assertNotIn("原因：持倉無需處理，新倉僅列可行動候選。", summary)
        self.assertNotIn("風險：新倉：尚未買入，不列入交易執行，分批且不追價", summary)
    ...
    def test_v20_0_14_post_market_fixture_uses_next_day_plan_semantics(self):
        ...
        self.assertIn("【05/28 盤後｜v21.1】", summary)
        self.assertIn("📌 盤後簡報", summary)
        self.assertIn("結論：新倉無有效進場；未持倉等觸發。", summary)
        self.assertIn("明日計畫：可準備 1 檔，開盤確認前不下單。", summary)
        self.assertNotIn("市場：中性觀察 R2", summary)
        self.assertIn("未持倉 1｜準備觀察 1（待確認）", summary)
        ...
        self.assertIn("進場：明日準備｜不可下單｜原因：盤後訊號需開盤確認", unheld)
        self.assertIn("缺口：盤後待開盤確認", unheld)
        self.assertIn("可買：開盤後守突破區 + 不追價", unheld)
        ...
        self.assertNotIn("今日盤中風控建議", summary)
        self.assertNotIn("明日計畫 1", summary)
    ...
    def test_v20_4_39_post_market_mixed_trend_and_prepare_keeps_trend_actionable(self):
        ...
        self.assertIn("【06/03 盤後｜v21.1】", summary)
        self.assertIn("結論：新倉候選 1 檔，明日開盤前確認。", summary)
        self.assertIn("明日計畫：新倉候選 1 檔，明日開盤前重新確認。", summary)
        self.assertIn("未持倉 2｜趨勢延續 1｜準備觀察 1（待確認）", summary)
        self.assertIn("新倉建議", summary)
        ...
        self.assertIn("進場：明日準備｜不可下單｜原因：盤後訊號需開盤確認", ordinary_card)
        self.assertIn("缺口：盤後待開盤確認", ordinary_card)
        self.assertIn("明日觸發：開盤後守突破區 + 不追價", ordinary_card)
    ...
    def test_telegram_messages_can_include_detail_when_requested(self):
        ...
        self.assertIn("【持倉標的】", position_message(messages))
        self.assertIn("【未持倉標的】", unheld_message(messages))
        self.assertIn(f"｜{generator.VERSION}】", summary_message(messages))
    ...
    def test_v20_4_12_complete_message_list_orders_holdings_unheld_summary_evidence_details(self):
        ...
        self.assertIn(f"【06/01 盤中｜{generator.VERSION}】", messages[0])
        self.assertIn(f"【06/01 盤中｜{generator.VERSION}】", messages[1])
        self.assertIn(f"【06/01 盤中｜{generator.VERSION}】", messages[2])
        ...
        self.assertIn(f"🧾 {generator.VERSION} 簡報", messages[2])
        self.assertEqual(messages[2].count("\n決策簡報\n"), 1)
        self.assertNotIn("\n資料依據\n", messages[2])
        self.assertNotIn("持倉：依第一則既有卡片處理，不新增第二個主行動。", messages[2])
        self.assertIn("新倉：無有效進場", messages[2])
        self.assertNotIn("策略樣本：缺少可驗證來源，本次不納入買賣判斷。", messages[2])
        for term in FORBIDDEN_SHORT_EVIDENCE_TERMS:
            self.assertNotIn(term, messages[2])

    def test_afterhours_cards_are_denoised_without_first_read_preface(self):
        holding_payload = render_payload(
            [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119],
            {"shares": 50, "avg_price": 118},
            price=108,
        ...
        holding_payload["today_action"] = "BUY"
        holding_payload["today_buy_shares"] = 50
        holding_payload["position_events"] = {"event_count": 1, "bought_shares": 50, "buy_price": 118}
        ...
        first = position_message(messages)
        holding_card = card_block(first, "【智原 3035】")
        ...
        self.assertIn("明日處理：", holding_card)
        self.assertNotIn("\n條件：", holding_card)
        self.assertNotIn("\n數據：", holding_card)
        self.assertNotIn("\n盤面：", unheld_card)
        self.assertIn("進場：不可買", unheld_card)
        self.assertIn("原因：", unheld_card)
        self.assertIn("缺口：", unheld_card)
        self.assertIn("明日觸發：", unheld_card)
        self.assertNotIn("\n原因：", unheld_card)
        self.assertNotIn("\n數據：", unheld_card)
        self.assertIs(messages[2], summary_message(messages))
        self.assertIn(f"🧾 {generator.VERSION} 簡報", messages[2])
    ...
    def test_v20_4_21_afterhours_mobile_readability_probe(self):
        ...
        self.assertIn("未持倉 2｜準備觀察 1（待確認）｜不可追高觀察 1（待確認）", brief)
        self.assertNotIn("資料依據", brief)
    ...
    def test_v20_4_24_weak_far_holding_shows_observation_days(self):
        ...
        self.assertIn("明日處理：弱勢觀察第 2 天；若第 3 天仍未重新接近買點 / 突破區，降低優先級", position)
    ...
    def test_v20_4_24_weak_far_holding_shows_position_events_observation_days(self):
        ...
        self.assertIn("明日處理：弱勢觀察第 3 天；若第 4 天仍未重新接近買點 / 突破區，降低優先級", position)
        self.assertNotIn("缺口：觀察：弱勢觀察", position)
    ...
    def test_v20_4_24_weak_far_holding_observation_days_missing_fails_closed(self):
        ...
        self.assertIn("【智原 3035】📌 續抱觀察", position)
        self.assertIn("明日處理：觀察天數未確認；若無法重新接近買點 / 突破區，降低優先級", position)
    ...
    def test_v20_4_10_complete_report_context_manifest_maps_visible_fields(self):
        ...
        self.assertEqual(context["report_context"]["version"], "v21.1")
        ...
        self.assertIn("🧾 v21.1 簡報", summary_message(messages))
        ...
        self.assertNotIn("Source：price available｜OHLCV available｜風險報酬 derived｜score derived｜volume derived", unheld_message(messages))
        ...
        for case in [
            ...
            with self.subTest(case=case):
                ...
                for artifact in report["artifacts"]:
                    ...
                    if artifact["source_type"] == "production-readonly" and artifact["status"] in {
                        "missing-source",
                        "insufficient-data",
                        "source-error",
                        "unresolved-conflict",
                        ...
                        self.assertTrue(
                            verifier.get("fail_closed_status") or verifier.get("conflict_fail_closed")
                        )
                    else:
                        self.assertTrue(verifier["pass"])
    ...
    def test_v20_4_10_manifest_fails_closed_for_missing_candidate_sources(self):
        ...
        self.assertIn("【05/31 盤中｜v21.1】", summary)
        ...
        self.assertNotIn("風控建議 1", summary)
        self.assertNotIn("建準｜可買", summary)
        self.assertNotIn("Source：漏斗 count", summary)
        self.assertIn("【建準 2421】⛔ 不可行動｜資料來源缺失", unheld)
        self.assertIn("進場：不可買，資料來源缺失", unheld)
        self.assertIn("資料：缺現價/OHLCV，停止新倉判斷", unheld)
        self.assertNotIn("Source：price insufficient-data｜OHLCV insufficient-data｜風險報酬 insufficient-data", unheld)
    ...
    def test_v20_4_10_summary_hides_strongest_when_candidate_source_missing(self):
        ...
        self.assertIn("進場：不可買，資料來源缺失", unheld)
        self.assertIn("資料：缺現價/OHLCV，停止新倉判斷", unheld)
        self.assertNotIn("Source：price missing-source｜OHLCV missing-source｜風險報酬 missing-source", unheld)
    ...
    def test_source_missing_no_empty_execution_placeholder_and_hides_all_zero_unheld_funnel(self):
        ...
        self.assertNotIn("今日盤中風控建議", brief)
        self.assertNotIn("今日盤中風控建議", rendered)
        self.assertNotIn("無新增下單", brief)
        self.assertNotIn("無新增下單", rendered)
        self.assertNotIn("交易執行：無新增下單", rendered)
        self.assertNotIn("未持倉狀態", brief)
    ...
    def test_presentation_noise_afterhours_normal_sources_hide_data_basis(self):
        ...
        self.assertIn("明日計畫：可準備 1 檔，開盤確認前不下單。", brief)
    ...
    def test_telegram_message_noise_consistency_intraday_owner_nine_points(self):
        ...
        self.assertNotIn("交易狀態：不可行動｜動作：停止行動", rejected_card)
        self.assertNotIn("風險報酬 3.06", rejected_card)
        self.assertNotIn("回測：", tracking_card)
        self.assertIn("未持倉 4｜僅追蹤 3（等冷卻）｜淘汰 1", brief)
        self.assertNotIn("回測：不可用", unheld)
        self.assertNotIn("樣本不足（有效樣本3）", unheld)
        self.assertIn("盤中觸發：降溫到 Lv.1 + 回測不破", partial_card)
        self.assertNotIn("證據：過熱不適用", partial_card)
    ...
    def test_0603_v20_4_32_failure_specimen_message_list_replay(self):
        ...
        self.assertIn("【06/03 盤中｜v21.1】", summary)
        ...
        self.assertIn("原因：同日建倉後跌破快速止損 / 入場價 3%，觸發當日減碼", umc_card)
        self.assertNotIn("條件：同日建倉後觸發快速止損 / 入場即錯", umc_card)
        self.assertIn("聯電｜-3.86%｜減碼", summary)
        self.assertNotIn("新倉風控觀察", umc_card + summary)
        self.assertIn("【技嘉 2376】⏳ 等冷卻｜過熱觀察", gigabyte_card)
        self.assertIn("盤中觸發：降溫到 Lv.1 + 回測不破", gigabyte_card)
        self.assertNotIn("數據：風險報酬 -（過熱）", gigabyte_card)
        self.assertNotIn("風險報酬 0.21", gigabyte_card)
        self.assertIn("【光寶科 2301】⛔ 不買｜前態待確認", unstable_card)
        self.assertNotIn("不買｜進場", rendered)
        self.assertEqual(len(reason_lines), 0)
        self.assertNotIn("回測：不可用", unheld)
        self.assertNotIn("回測：-", unheld)
        self.assertNotIn("樣本不足（有效樣本3）", unheld)
    ...
    def test_v20_4_36_non_actionable_unheld_hides_score_numbers(self):
        ...
        self.assertIn("風險報酬 -（量能不足）", low_volume_card)
    ...
    def test_0604_v20_4_36_mobile_readability_message_list_replay(self):
        ...
        self.assertIn("今日買入紀錄 3（已風控 2/觀察 1）", summary)
        ...
        self.assertNotIn("不適用（不可行動）｜證據：風控不適用", failed_card)
        self.assertNotIn("不適用（過熱）", failed_card)
        self.assertIn("淘汰", gigabyte_card)
        self.assertNotIn("證據：風控不適用", gigabyte_card)
    ...
    def test_0604_v20_4_37_generate_mobile_consistency_message_list_replay(self):
        ...
        self.assertIn("未持倉 8｜準備觀察 3（待確認）｜不可追高觀察 1（待確認）｜僅追蹤 2", summary)
    ...
    def test_eight_day_confirmed_market_theme_is_decision_eligible(self):
        ...
        self.assertEqual(generator.VERSION, "v21.1")
    ...
    def test_per_stock_backtest_context_drives_strategy_ready_when_global_sample_partial(self):
        ...
        rendered = "\n\n".join(messages)
        self.assertIn("【06/03 盤中｜v21.1】", rendered)
    ...
    def test_setup_strategy_summary_drives_per_stock_modifier_and_rendered_cards(self):
        ...
        self.assertNotIn("證據：風控不適用", umc_card)
    ...
    def test_confirmed_evidence_near_boundary_adjusts_prepare_and_report_reason(self):
        ...
        self.assertIn("未持倉 1｜準備觀察 1（待確認）", summary)
        self.assertIn("未持倉 1 檔可準備，等觸發，不列入今日盤中風控建議", summary)
        self.assertNotIn("📎 詳情索引：", summary)
        self.assertIn("【近門檻 9999】👀 準備觀察｜突破回測", card)
    ...
    def test_mixed_evidence_adjusted_and_ordinary_prepare_split_counts_and_labels(self):
        ...
        self.assertIn("未持倉 2｜準備觀察 1（待確認）｜不可追高觀察 1（待確認）", summary)
        self.assertIn("準備觀察 1（待確認）｜不可追高觀察 1（待確認）", summary)
        self.assertNotIn("📎 詳情索引：", summary)
        self.assertNotIn("未持倉 2 檔可準備", summary)
        self.assertNotIn("可準備 2（不可買）", summary)
        self.assertIn("【證據調整 9999】👀 準備觀察｜突破回測", adjusted_card)
    ...
    def test_v20_4_42_postmarket_unheld_gate_attribution_readability_message_list_replay(self):
        ...
        self.assertIn("距突破：6%｜遠離突破", rr_card)
        self.assertNotIn("買點區<=5%", rr_card)
        self.assertIn("可買：風險報酬 >= 1.5", rr_card)
        self.assertNotIn("盤後待確認", rr_card)
        self.assertIn("【仁寶 2324】👀 等風險報酬｜風險報酬不足", rr_near_card)
        self.assertIn("進場：不買，等風險報酬達標｜原因：風險報酬還不夠", rr_near_card)
        ...
        self.assertIn("狀態：短線過熱，先等冷卻", hot_card)
        self.assertIn("盤中觸發：降溫到 Lv.1 + 回測不破", hot_card)
        self.assertNotIn("等待：熱度 Lv.2；有效買點：", hot_card)
        ...
        self.assertIn("進場：不可買，等重新轉強｜原因：尚未站回突破區", failed_card)
        self.assertIn("距突破：9%｜遠離突破", failed_card)
        self.assertIn("缺口：尚未站回突破區", failed_card)
        self.assertIn("可買：重新站回突破區後再評估", failed_card)
        self.assertNotIn("風險報酬 0｜需>=1.5", failed_card)
        self.assertNotIn("風險報酬不可用 / 不作主因", failed_card)
        self.assertIn("【光寶科 2301】🟡 明日準備｜不可買｜開盤後確認", prepare_card)
        self.assertIn("進場：明日準備｜不可下單｜原因：盤後訊號需開盤確認", prepare_card)
        self.assertIn("缺口：盤後待開盤確認", prepare_card)
        self.assertIn("可買：開盤後守突破區 + 不追價", prepare_card)
        ...
        self.assertNotIn("原因：", buyable_card)
        self.assertNotIn("缺口：", buyable_card)
        self.assertNotIn("距突破 0.5%/需<=4%", buyable_card)
        self.assertNotIn("買點距離已觸發/不追價", buyable_card)
        self.assertNotIn("原因：", trend_card)
        self.assertNotIn("缺口：", trend_card)
        self.assertNotIn("原因：", actionable_trend_card)
        self.assertNotIn("缺口：", actionable_trend_card)
    ...
    def test_confirmed_evidence_preserves_limit_lock_chase_hard_blocker(self):
        ...
        self.assertNotIn("準備觀察 1（待確認）", summary)
        self.assertIn("【追高 9999】⏳ 等回測｜漲停不追", card)
        self.assertIn("狀態：漲停/過熱，不追價", card)
        self.assertIn("等待：解除鎖定後，看開板回測是否守住", card)
        self.assertIn("有效買點：開板/降溫 + 回測不破 + 非追高", card)
        self.assertNotIn("等風險報酬", card)
        self.assertNotIn("缺口：解除鎖定", card)
        self.assertNotIn("數據：風險報酬", card)
        self.assertNotIn("綜合", card)
        ...
        self.assertNotIn("風險報酬 -（過熱）｜不適用（過熱）｜證據：過熱不適用", card)
    ...
    def test_v20_4_43_evidence_chain_decision_judgments_cover_eligibility_layers(self):
        ...
        self.assertIn("【06/04 盤中｜v21.1】", summary_message(messages))
    ...
    def test_overheat_pullback_display_switches_from_cooling_to_retest(self):
        ...
        self.assertIn("觸發：回測不破 + 非追高 + 量能不失控", lines)
        self.assertFalse(any("等待：熱度" in line for line in lines))
        self.assertFalse(any(line.startswith("有效買點：") for line in lines))
        ...
        self.assertIn("盤中觸發：回測不破 + 非追高 + 量能不失控", card)
    ...
    def test_overheat_sharp_pullback_display_focuses_on_support(self):
        ...
        self.assertIn("觸發：止跌守支撐 + 量能不失控", lines)
        self.assertFalse(any(line.startswith("觀察：") for line in lines))
