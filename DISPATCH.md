...
- task_md_task_id: `fix-v20-4-37-rr-insufficient-message-readability`
...
- `v20.4.38`｜`fix-v20-4-37-rr-insufficient-message-readability`｜normal_patch｜修復 RR不足 / 等RR修復 報文誤讀：光寶科類卡片不再把 RR不足寫成 `證據：資料不足`，改為 `原因：RR不足，等待RR修復`；summary 回測摘要排除僅追蹤 / 等RR修復標的，不再顯示 `回測（光寶科）`，但保留建準候選回測；版本升 `v20.4.38`；official message-list replay 4 passed、py_compile / diff check passed、QA 補 formatter probe `通過`；未改 RR 公式、strategy decision、DB schema/write、live Telegram；下一步收口 commit / push / git completion gate。
...
- 本輪 v20.4.38 RR不足 / 等RR修復 報文可讀性修復已 QA 通過；下一步收口 commit / push / git completion gate。
