...
- 狀態：correction audit fail-closed 與 `daily_signal_snapshot` 歷史語義修復已完成；production market/theme 五月 coverage 仍 blocked。
...
- 未完成前不得把 latest-only source 稱為五月歷史；下一步需另開資料修復任務抓 market/theme 五月歷史並處理 confirmed evidence duplicate groups。
...
- 證據鏈下一步不得直接繼續功能擴張；先定義並執行 market/theme historical coverage 與 confirmed evidence dedupe 任務。`daily_signal_snapshot` 不做舊五月 current-version backfill，只驗後續 runner 每日正常寫入。
