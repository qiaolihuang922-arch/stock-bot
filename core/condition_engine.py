...
def condition_engine(result):
    ...
    if price_behavior in ["LIMIT_LOCK", "LIMIT_REBOUND"]:
        # 中文註釋：v19.1.3 漲停鎖價 / 漲停反彈是已辨識價格行為，不再顯示事件與 Edge 全缺。
        conditions["event"] = True
        conditions["edge"] = True

    if structure_phase in ["SHAKEOUT", "HEALTHY_PULLBACK"]:
        # 中文註釋：v19.1.3 洗盤 / 健康回踩屬於持倉觀察條件，量能弱不直接視為交易錯誤。
        conditions["event"] = True
        conditions["edge"] = True

    if entry_quality in ["A+", "A", "B"]:
        # 中文註釋：v19.1.3 入場品質達 B 以上代表證據鏈已成形，條件映射不再只看單一 RR。
        conditions["edge"] = True
    ...
    if decision_type == "watch_quality_c":
        # 中文註釋：v19.1.3 C 品質觀察是已辨識的策略狀態，RR 足夠時不再顯示 RR 不足。
        return {
            **conditions,
    ...
    if decision_type in [
        ...
        # 中文註釋：v19.1.3 假突破 / 過熱屬於已辨識事件，不再顯示事件與 Edge 全缺。
        conditions["event"] = True
        conditions["edge"] = True
